...
class reposit
{
    ...
    function SelectCondTrueSaf($config)
    {
        $conf = explode("|", $config);
        $args = $conf[1];
        $args = $this->anti_injection($args);
        $sql = "SELECT * FROM " . '[Saf].' . $conf[0] . " WHERE "
            . $conf[1];
    ...
    function InsertValores($config)
    {
        ...
        $sql = "insert into " . $conf[0] . "(" . $listacampos
            . ") values("
            . $listavalores . ") ";
    ...
    function Delete($config)
    {
        ...
        $conf[1] = $this->anti_injection($conf[1]);
        $sql = "DELETE FROM "
            // . '[' . self::SCHEMA . ']. ' // segui o padrao do update 25/07/2022-molina
            . $conf[0] . " WHERE " . $conf[1]
            . " ";
    ...
    function PossuiPermissao($config)
    {
        ...
        if ($row = $result[0]) {
            ...
            if ($grupo) {
                ...
                $sql = $sql . " WHERE usuf.grupo=" . $grupo . " ";
